...
public class AddRecipe extends Activity implements DialogInterface.OnClickListener {
    ...
    private void addRecipe(String recipeName,String recipeUrl,String course,String ingredient){
    	DatabaseHelper rec = new DatabaseHelper(this);
        ContentValues values = new ContentValues();
        if(recipeName != null){
            values.put(DatabaseHelper.COL_RECIPE_NAME, recipeName);
        }
        if(recipeUrl != null){
            values.put(DatabaseHelper.COL_RECIPE_URL, recipeUrl);
        }
        values.put(DatabaseHelper.COL_RECIPE_COURSE, course);
        values.put(DatabaseHelper.COL_RECIPE_INGREDIENT, ingredient);
        try {
